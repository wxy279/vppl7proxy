...
tcp_connection_reset (tcp_connection_t * tc)
{
  ...
  switch (tc->state)
    ...
    case TCP_STATE_LAST_ACK:
      ...
      session_transport_closed_notify (&tc->connection, WAITCLOSE_TIMEO_REASON_NON_EST_RESET_COMING);
	  session_transport_reset_notify (&tc->connection);
      tcp_connection_set_state (tc, TCP_STATE_CLOSED);
      break;
...
tcp_connection_close (tcp_connection_t * tc)
{
  ...
  switch (tc->state)
      ...
      if (transport_max_rx_dequeue (&tc->connection))
	{
	  tcp_send_reset (tc);
	  tcp_connection_timers_reset (tc);
	  tcp_stack_printf("Closing with unread data, reset the connection. (local %d : remote %d)",
	  		clib_net_to_host_u16(tc->c_lcl_port), clib_net_to_host_u16(tc->c_rmt_port));
	  tcp_connection_set_state (tc, TCP_STATE_CLOSED);
	  tcp_timer_set (tc, TCP_TIMER_WAITCLOSE, TCP_CLOSEWAIT_TIME);
	  break;
	}
      if (!stream_session_bytes_in_tbs_storage (&tc->connection)) {
		tcp_send_fin (tc);
		tcp_stack_printf("TCP_STATE_ESTABLISHED close case: No tbs buffer, send FIN directly, and step to TCP_STATE_FIN_WAIT_1. (local %d : remote %d)",
	  		clib_net_to_host_u16(tc->c_lcl_port), clib_net_to_host_u16(tc->c_rmt_port));
      } else {
	  	tcp_stack_printf("TCP_STATE_ESTABLISHED close case: There tbs buffer, set flag TCP_CONN_FINPNDG, and step to TCP_STATE_FIN_WAIT_1. (local %d : remote %d)",
	  		clib_net_to_host_u16(tc->c_lcl_port), clib_net_to_host_u16(tc->c_rmt_port));
		tc->flags |= TCP_CONN_FINPNDG;
      }
      tcp_connection_set_state (tc, TCP_STATE_FIN_WAIT_1); /*不管是不是把FIN的发送Pending了都进入FIN_1状态*/
	...
	{
	  tcp_send_fin (tc);
	  tcp_stack_printf("TCP_STATE_CLOSE_WAIT close case: No tbs buffer, send FIN directly, and step to TCP_STATE_LAST_ACK. (local %d : remote %d)",
	  		clib_net_to_host_u16(tc->c_lcl_port), clib_net_to_host_u16(tc->c_rmt_port));
	  tcp_connection_timers_reset (tc);
	  tcp_connection_set_state (tc, TCP_STATE_LAST_ACK); /*只有没有发送缓存了才会进入LASK_ACK状态*/
	  tcp_timer_update (tc, TCP_TIMER_WAITCLOSE, TCP_2MSL_TIME);
	} else {
		tcp_stack_printf("TCP_STATE_CLOSE_WAIT close case: There tbs buffer, set flag TCP_CONN_FINPNDG,, and keep in TCP_STATE_CLOSE_WAIT. (local %d : remote %d)",
	  		clib_net_to_host_u16(tc->c_lcl_port), clib_net_to_host_u16(tc->c_rmt_port));
		tc->flags |= TCP_CONN_FINPNDG;	/*发送缓存不空，则keep in close_wait状态并且加上FINPNDG标志*/
      }
      break;
    case TCP_STATE_FIN_WAIT_1:
      tcp_timer_update (tc, TCP_TIMER_WAITCLOSE, TCP_2MSL_TIME);
...
static void
tcp_session_terminate(u32 conn_index, u32 thread_index, u16 rst_id)
{
	tcp_connection_t *tc;
	tc = tcp_connection_get (conn_index, thread_index);
	//tcp_send_reset(tc, rst_id);
	tcp_send_reset_with_id(tc, rst_id);
...
tcp_session_flush_data (transport_connection_t * tconn)
{
  ...
  tc->psh_seq = tc->snd_una + stream_session_bytes_in_tbs_storage (tconn) - 1;
}
...
const static transport_proto_vft_t tcp_proto = {
  ...
  .terminate = tcp_session_terminate,
...
tcp_timer_waitclose_handler (u32 conn_index)
{
  ...
  switch (tc->state)
    {
    /*收到了FIN以后 就进入CLOSE_WAIT状态，如果此处超时代表要么FIN没法出去或者根本没人调用session_close*/
    case TCP_STATE_CLOSE_WAIT:
      tcp_connection_timers_reset (tc);
      session_transport_closed_notify (&tc->connection, WAITCLOSE_TIMEO_REASON_CLOSE_WAIT_TOO_LONG);
	  tcp_stack_printf("waitclose_timer handler TCP_STATE_CLOSE_WAIT case. (local %d : remote %d)",
	  		clib_net_to_host_u16(tc->c_lcl_port), clib_net_to_host_u16(tc->c_rmt_port));
	/* Session didn't come back with a close.*/
      if (!(tc->flags & TCP_CONN_FINPNDG))
	{ /*通过FINPNDG判断的依据是，如果在close_wait状态下，当调用过了session_close则要么有该标志，要么状态转变为LAST_ACK
       * 所以这里没有这个标志就说明肯定是应用层根本没有调用session_close
	   */
	  /*TCP_CLOSEWAIT_TIME 2s内app没有调用session_close 直接准备删除 够狠！*/
	  tcp_stack_printf("waitclose_timer handler TCP_STATE_CLOSE_WAIT case: APP no response for transport closing event for long timer, step to TCP_STATE_CLOSED. (local %d : remote %d)",
	  		clib_net_to_host_u16(tc->c_lcl_port), clib_net_to_host_u16(tc->c_rmt_port));
	  tcp_connection_set_state (tc, TCP_STATE_CLOSED);/*reenter this function and call tcp_connection_del*/
	  tcp_timer_set (tc, TCP_TIMER_WAITCLOSE, TCP_CLEANUP_TIME);
	  break;
	}

      /* Passive close case, and session_close is called but data not empty(in tcp_connection_close),
       * only set the TCP_CONN_FINPNDG, now it's timeout(maybe all data sent),force send FIN either way
       * and switch to LAST_ACK. */
      tcp_stack_printf("waitclose_timer handler TCP_STATE_CLOSE_WAIT case: TCP_CONN_FINPNDG is set, now force to send FIN out. (local %d : remote %d)",
	  		clib_net_to_host_u16(tc->c_lcl_port), clib_net_to_host_u16(tc->c_rmt_port));
      ...
      break;
    case TCP_STATE_FIN_WAIT_1:
	  /*TCP_FINWAIT1_TIME 60s, tcp_connection_close change est state to fin1 and set 60s timer*/
      tcp_connection_timers_reset (tc);
	  tcp_stack_printf("waitclose_timer handler TCP_STATE_FIN_WAIT_1 case. (local %d : remote %d)",
	  		clib_net_to_host_u16(tc->c_lcl_port), clib_net_to_host_u16(tc->c_rmt_port));
      if (tc->flags & TCP_CONN_FINPNDG)
	{/*tcp_connection_close 时发现发送缓存不为空，进入FIN_WAIT_1状态后设置这个标志*/
	  /* If FIN pending, we haven't sent everything(if sent both data and fin out, when fin acked
	   * the state should be), but we did try.
	   * Notify session layer that transport is closed. */
	   /*一直有数据在发送，或者没有发送完，或者发送了 对端没有给ACK,或者在tcp46_rcv_process_inline中
        * 发现了数据已经被ACK，并且发送了FIN出去，但是没有得到对端的ACK 让状态跃迁到FIN_WAIT_2
	   */
	  tcp_stack_printf("waitclose_timer handler TCP_STATE_FIN_WAIT_1 case: TCP_CONN_FINPNDG is set, but don't move out from this state, step to TCP_STATE_CLOSED. (local %d : remote %d)",
	  		clib_net_to_host_u16(tc->c_lcl_port), clib_net_to_host_u16(tc->c_rmt_port));
	  tcp_connection_set_state (tc, TCP_STATE_CLOSED);
	  session_transport_closed_notify (&tc->connection, WAITCLOSE_TIMEO_REASON_FIN_WAIT_1_TOO_LONG);
	  tcp_send_reset (tc);
	  tcp_timer_set (tc, TCP_TIMER_WAITCLOSE, TCP_CLEANUP_TIME);
	}
      else
	{
	  ...
	  tcp_stack_printf("waitclose_timer handler TCP_STATE_FIN_WAIT_1 case: send FIN to peer, but peer side no response. (local %d : remote %d)",
	  		clib_net_to_host_u16(tc->c_lcl_port), clib_net_to_host_u16(tc->c_rmt_port));
	  session_transport_closed_notify (&tc->connection, WAITCLOSE_TIMEO_REASON_FIN_WAIT_1_NO_RESPONSE);
	  tcp_connection_set_state (tc, TCP_STATE_CLOSED);
	  tcp_timer_set (tc, TCP_TIMER_WAITCLOSE, TCP_CLEANUP_TIME);
	}
      break;
    case TCP_STATE_LAST_ACK:
    case TCP_STATE_CLOSING:
	  tcp_stack_printf("waitclose_timer handler TCP_STATE_CLOSING or TCP_STATE_LAST_ACK case. (local %d : remote %d)",
	  		clib_net_to_host_u16(tc->c_lcl_port), clib_net_to_host_u16(tc->c_rmt_port));
      tcp_connection_timers_reset (tc);
      tcp_connection_set_state (tc, TCP_STATE_CLOSED);
      tcp_timer_set (tc, TCP_TIMER_WAITCLOSE, TCP_CLEANUP_TIME);
      session_transport_closed_notify (&tc->connection, WAITCLOSE_TIMEO_REASON_LAST_ACK_AND_CLOSING);
      break;
    default:
...
static timer_expiration_handler *timer_expiration_handlers[TCP_N_TIMERS] =
{
    ...
    tcp_timer_establish_handler,		/* SYN_RCV establish */
    tcp_timer_establish_ao_handler,		/* Active Open establish timer, set by tcp_send_syn*/
};
/* *INDENT-ON* */
