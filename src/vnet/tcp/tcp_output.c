...
tcp_make_synack_options (tcp_connection_t * tc, tcp_options_t * opts)
{
  ...
  if (tcp_opts_sack_permitted (&tc->rcv_opts) && TCP_USE_SACKS)
    {
      opts->flags |= TCP_OPTS_FLAG_SACK_PERMITTED;
      len += TCP_OPTION_LEN_SACK_PERMITTED;
    }
 ...
 */
void
tcp_send_reset_with_id(tcp_connection_t * tc, u16 rst_id)
{
  tcp_worker_ctx_t *wrk = tcp_get_worker (tc->c_thread_index);
  vlib_main_t *vm = wrk->vm;
  vlib_buffer_t *b;
		     ...
		     u32 offset, u32 max_deq_bytes, vlib_buffer_t ** b)
{
  ...
  if (PREDICT_TRUE (seg_size <= bytes_per_buffer))
	...
	return 0;
      *b = vlib_get_buffer (vm, bi);
      ...
      n_bytes = stream_session_peek_bytes_from_tbs_storage (&tc->connection, data, offset,
					    max_deq_bytes);
  ...
  /* Split mss into multiple buffers */
      ...
      for (i = 1; i < n_bufs_per_seg; i++)
	{
	  ...
	  n_peeked = stream_session_peek_bytes_from_tbs_storage (&tc->connection, data,
						 offset + n_bytes,
						 len_to_deq);
				...
				u32 max_deq_bytes, vlib_buffer_t ** b)
{
   ...
   */
  available_bytes = stream_session_bytes_in_tbs_storage (&tc->connection);
			  ...
			  u32 burst_size)
{
  ...
  max_deq = stream_session_bytes_in_tbs_storage (&tc->connection);
