...
void tcp_send_reset_with_id(tcp_connection_t * tc, u16 rst_id);
...
extern int tcp_stack_debug;

#define tcp_stack_printf(format, args...) do {		\
	if (tcp_stack_debug) {							\
...
#define tcp_validate_txf_size(_tc, _a) 					\
  ASSERT(_tc->state != TCP_STATE_ESTABLISHED 				\
	 || stream_session_bytes_in_tbs_storage (&_tc->connection) >= _a)
