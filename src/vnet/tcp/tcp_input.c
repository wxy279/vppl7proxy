...
/*define in session.h and used by hili*/
...
tcp_options_parse (tcp_header_t * th, tcp_options_t * to, u8 is_syn)
{
  ...
  for (; opts_len > 0; opts_len -= opt_len, data += opt_len)
	...
	case TCP_OPTION_SACK_PERMITTED:
	  if (!is_syn)
	    break;
	  if (opt_len == TCP_OPTION_LEN_SACK_PERMITTED && tcp_syn (th) && TCP_USE_SACKS)
...
tcp_handle_postponed_dequeues (tcp_worker_ctx_t * wrk)
{
  ...
  for (i = 0; i < vec_len (pending_deq_acked); i++)
	...
	continue;
	  ...
	  stream_session_drop_bytes_from_tbs_storage(&tc->connection, tc->burst_acked);
      tc->burst_acked = 0;
...
tcp_update_snd_wnd (tcp_connection_t * tc, u32 seq, u32 ack, u32 snd_wnd)
{
  ...
  if (seq_lt (tc->snd_wl1, seq)
	...
	{
	  stream_session_window_update_notify(&tc->connection);
			  ...
			  u16 data_len)
{
	...
	/*When the app gone, the wrtiten len returned is 0, but it no meaning*/
	if (/*(written == 0) && */ is_stream_session_hili_proxy_enable()) {
		written = data_len;
	}
...
always_inline void
vlib_buffer_free_if_no_using(vlib_main_t * vm,
		  /* pointer to first buffer */
		  u32 * buffers,
		  /* number of buffers to free */
		  u32 n_buffers)
{
	u32 *real_free_buffers = 0;
	u32 n_real_free_buffers = 0;
	vlib_buffer_t *cur_vb;
	int  i;

	for (i = 0; i < n_buffers; i++) {
		cur_vb = vlib_get_buffer(vm, buffers[i]);
		if (cur_vb->flags & VNET_BUFFER_F_SESSION_HOLDING) {
			cur_vb->flags &= ~ VNET_BUFFER_F_SESSION_HOLDING;
		} else {
			vec_add1(real_free_buffers, buffers[i]);
			n_real_free_buffers += 1;
		}
	}

	if (vec_len(real_free_buffers) != n_real_free_buffers) {
		clib_warning("Something don't know happen ?");
		return;
	}

	if (n_real_free_buffers > 0) {
		vlib_buffer_free(vm, real_free_buffers, n_real_free_buffers);
	}
			  ...
			  vlib_frame_t * frame, int is_ip4)
{
  ...
  while (n_left_from > 0)
      ...
      if (PREDICT_FALSE (tcp_segment_validate (wrk, tc0, b0, th0, &error0)))
	{
	  ...
	  /* tcp_rcv_ack 内部会确定此ACK burst_acked数量，而该值确定送发送缓存去掉多少字节的数据*/
  ...
  if (!use_copy_when_recv) {
  	vlib_buffer_free_if_no_using(vm, first_buffer, frame->n_vectors);
  } else {
			  ...
			  vlib_frame_t * from_frame, int is_ip4)
{
  ...
  while (n_left_from > 0)
	...
	case TCP_STATE_FIN_WAIT_1:
	  ...
	  if (tc0->flags & TCP_CONN_FINPNDG)
	    { /*处于FIN_WAIT_1 且 FIN is pending,这里收到了ACK后看下是否ACK了所有数据，如果是happy地发送FIN*/
	      /* TX fifo finally drained */
		  tcp_stack_printf("TCP_STATE_FIN_WAIT_1 check ack case: TCP_CONN_FINPNDG is set. (local %d : remote %d)",
	  		clib_net_to_host_u16(tc0->c_lcl_port), clib_net_to_host_u16(tc0->c_rmt_port));
	      max_dequeue = stream_session_bytes_in_tbs_storage (&tc0->connection);
	      if (max_dequeue <= tc0->burst_acked) {
		  	tcp_stack_printf("TCP_STATE_FIN_WAIT_1 check ack case: TCP_CONN_FINPNDG is set, and happy to know all acked and start to send FIN. (local %d : remote %d)",
	  		clib_net_to_host_u16(tc0->c_lcl_port), clib_net_to_host_u16(tc0->c_rmt_port));
			tcp_send_fin (tc0);  /*发送FIN出去 就代表可以进入FIN_WAIT_2状态*/
	      /* If a fin was received and data was acked extend wait */
	      } else if ((tc0->flags & TCP_CONN_FINRCVD) && tc0->bytes_acked) {
			/*已经收到了对端的FIN，没有进入到同时关闭的原因是，本端的FIN实际上没有发出去，既然数据在不断的
			  被ACK，所以这里extend wait;
		    */
	  ...
	  /* If FIN is ACKed */
		...
		{
		  tcp_stack_printf("TCP_STATE_FIN_WAIT_1 check ack case: TCP_CONN_FINRCVD is set. (local %d : remote %d)",
	  		clib_net_to_host_u16(tc0->c_lcl_port), clib_net_to_host_u16(tc0->c_rmt_port));
		  tcp_connection_set_state (tc0, TCP_STATE_CLOSED);
		  tcp_timer_set (tc0, TCP_TIMER_WAITCLOSE, TCP_CLEANUP_TIME);
		  goto drop;
		}
		  tcp_stack_printf("TCP_STATE_FIN_WAIT_1 check ack case: FIN is ACKed, step to TCP_STATE_FIN_WAIT_2. (local %d : remote %d)",
	  		clib_net_to_host_u16(tc0->c_lcl_port), clib_net_to_host_u16(tc0->c_rmt_port));
	      ...
	      if (tc0->burst_acked > 1) {
			stream_session_drop_bytes_from_tbs_storage(&tc0->connection, tc0->burst_acked - 1);
		  }
	      tc0->burst_acked = 0;
	    }
	  break;
	...
	case TCP_STATE_CLOSE_WAIT:
	  ...
	  /*CLOSE_WAIT状态收到了ACK，要看是否是FIN pending了，如果是则代表有可能是数据部分被ACK了
	   * 则有可能可以发送FIN了， 在这个状态只有两种可能，要么是没有session_close，要么就是pending
	   */
	  tcp_stack_printf("TCP_STATE_CLOSE_WAIT check ack case. (local %d : remote %d)",
	  		clib_net_to_host_u16(tc0->c_lcl_port), clib_net_to_host_u16(tc0->c_rmt_port));
	  if (!(tc0->flags & TCP_CONN_FINPNDG))
	    break;

	  tcp_stack_printf("TCP_STATE_CLOSE_WAIT check ack case, FIN is pending. (local %d : remote %d)",
	  		clib_net_to_host_u16(tc0->c_lcl_port), clib_net_to_host_u16(tc0->c_rmt_port));
	  /*接下来认为是FIN pending了*/
	  /* Still have outstanding tx data，其实应该和FIN_1一样判断burst_acked 是否覆盖了所有的tx buffer*/
	  /* 这里的这个判断，意味着FIN同时tx buffer不空的case下（即fin pending)，下面的代码正常情况下是不能被
	  执行到了，除非对端对对同一个数据的ACK发送了两次，这样正常情况都要到tcp_timer_waitclose_handler去发送
	  第二个FIN去断连*/
	  if (stream_session_bytes_in_tbs_storage (&tc0->connection)) /*这里没有像上面的FIN_1那样做判断*/
	    break;

	   tcp_stack_printf("TCP_STATE_CLOSE_WAIT check ack case, there bug, can't come here. (local %d : remote %d)",
	  		clib_net_to_host_u16(tc0->c_lcl_port), clib_net_to_host_u16(tc0->c_rmt_port));
	  tcp_send_fin (tc0);
	  tcp_connection_timers_reset (tc0);
	...
	case TCP_STATE_LAST_ACK:
	  ...
	  tcp_stack_printf("TCP_STATE_LAST_ACK check ack case. (local %d : remote %d)",
	  		clib_net_to_host_u16(tc0->c_lcl_port), clib_net_to_host_u16(tc0->c_rmt_port));
	  ...
	  tcp_stack_printf("TCP_STATE_LAST_ACK check ack case: step to TCP_STATE_CLOSED. (local %d : remote %d)",
	  		clib_net_to_host_u16(tc0->c_lcl_port), clib_net_to_host_u16(tc0->c_rmt_port));
	...
	case TCP_STATE_ESTABLISHED:
	  /* Account for the FIN and send ack */
	  tcp_stack_printf("TCP_STATE_ESTABLISHED process FIN case: step to TCP_STATE_CLOSE_WAIT. (local %d : remote %d)",
	  		clib_net_to_host_u16(tc0->c_lcl_port), clib_net_to_host_u16(tc0->c_rmt_port));
	  tc0->rcv_nxt += 1;
	  tcp_program_ack (wrk, tc0);
	...
	case TCP_STATE_FIN_WAIT_1:
	  tc0->rcv_nxt += 1;
	  tcp_stack_printf("TCP_STATE_FIN_WAIT_1 process FIN case. (local %d : remote %d)",
	  		clib_net_to_host_u16(tc0->c_lcl_port), clib_net_to_host_u16(tc0->c_rmt_port));
	  if (tc0->flags & TCP_CONN_FINPNDG)
	    {
		  ...
		  tcp_stack_printf("TCP_STATE_FIN_WAIT_1 process FIN case: local FIN pending and receive remote FIN, set TCP_CONN_FINRCVD. (local %d : remote %d)",
	  		clib_net_to_host_u16(tc0->c_lcl_port), clib_net_to_host_u16(tc0->c_rmt_port));
	      tcp_timer_update (tc0, TCP_TIMER_WAITCLOSE, TCP_CLOSEWAIT_TIME);
	    }
	  else
	    {
	      tcp_stack_printf("TCP_STATE_FIN_WAIT_1 process FIN case: sametime closing case, step to TCP_STATE_CLOSING. (local %d : remote %d)",
	  		clib_net_to_host_u16(tc0->c_lcl_port), clib_net_to_host_u16(tc0->c_rmt_port));
	  ...
	  stream_session_peer_passive_disconnect_notify(&tc0->connection);
	  break;
	case TCP_STATE_FIN_WAIT_2:
	  /* Got FIN, send ACK! Be more aggressive with resource cleanup */
	  tcp_stack_printf("TCP_STATE_FIN_WAIT_2 process FIN case: good FIN and step to TCP_STATE_TIME_WAIT. (local %d : remote %d)",
	  		clib_net_to_host_u16(tc0->c_lcl_port), clib_net_to_host_u16(tc0->c_rmt_port));
	  tc0->rcv_nxt += 1;
	  tcp_connection_set_state (tc0, TCP_STATE_TIME_WAIT);
	  tcp_connection_timers_reset (tc0);
	...
	case TCP_STATE_TIME_WAIT:
	   ...
	   * timeout.
	   */
	   tcp_stack_printf("TCP_STATE_TIME_WAIT process FIN case. (local %d : remote %d)",
	  		clib_net_to_host_u16(tc0->c_lcl_port), clib_net_to_host_u16(tc0->c_rmt_port));
	  tcp_timer_update (tc0, TCP_TIMER_WAITCLOSE, TCP_TIMEWAIT_TIME);
	  break;
	}
  ...
  if (!use_copy_when_recv) {
  	vlib_buffer_free_if_no_using(vm, first_buffer, from_frame->n_vectors);
  } else {
	vlib_buffer_free (vm, first_buffer, from_frame->n_vectors);
  }
