...
always_inline u16
session_node_peek_data_from_tbs_storage(session_tx_context_t * ctx, u32 len_to_deq, u8 *copy_here)
{
	FUNC_TRACE;
	int copy_ret;
	u32 off_of_tbs;
	...
	if (is_stream_session_hili_proxy_enable()) {
		vb = hili_session_get_tbs_buffer(ctx->s, ctx->tx_offset, len_to_deq, &off_of_tbs);
		 ...
		 *byte to copy_here;
		 */
		//clib_memcpy(copy_here, vb->data + vb->current_data + off_of_tbs, len_to_deq);
		hili_buffer_trace_print(vb);
		copy_ret = vlib_buffer_hili_copydata(vm, vb, off_of_tbs, len_to_deq, copy_here);
		if (copy_ret == -1) {
			LINE_TRACE;
			ft_printf("vlib_buffer_hili_copydata return -1 !!\n");
			return 0;
		}
		return len_to_deq;
	} else {
		LINE_TRACE;
		n_bytes_read = svm_fifo_peek (ctx->s->tx_fifo, ctx->tx_offset, len_to_deq, copy_here);
		return n_bytes_read;
	}
}
  ...
  for (j = 1; j < ctx->n_bufs_per_seg; j++)
    {
      ...
      *n_bufs -= 1;
      ...
      if (peek_data)
	{
	  //n_bytes_read = svm_fifo_peek (ctx->s->tx_fifo, ctx->tx_offset, len_to_deq, data);
	  vlib_buffer_hili_set_alloc_module_id(chain_b, 2000);
	  n_bytes_read = session_node_peek_data_from_tbs_storage(ctx, len_to_deq, data);
			...
			vlib_buffer_t * b, u16 * n_bufs, u8 peek_data)
{
  ...
  if (peek_data)
    {
      //n_bytes_read = svm_fifo_peek (ctx->s->tx_fifo, ctx->tx_offset, len_to_deq, data0);
      vlib_buffer_hili_set_alloc_module_id(b, 2000);
	  n_bytes_read = session_node_peek_data_from_tbs_storage(ctx, len_to_deq, data0);
      ASSERT (n_bytes_read > 0);
			       ...
			       u32 max_segs, u8 peek_data)
{
  ...
  ctx->max_dequeue = stream_session_bytes_in_tbs_storage(ctx->tc);
  if (peek_data)
    {
...
hili_add_session_to_tcp_pending_txq(session_worker_t *wrk, session_t *hili_ss)
{
	...
	evt->session_handle = session_handle (hili_ss);
	evt->event_type = FIFO_EVENT_APP_DIRECT_TX;
}
#endif
  ...
  if (PREDICT_FALSE ((rv = session_tx_not_ready (ctx->s, peek_data))))
    {
	  ...
	  if (is_stream_session_hili_proxy_enable()) {
		session_node_print("Come here");
		hili_add_session_to_tcp_pending_txq(wrk, ctx->s);
	  } else {
...
always_inline void
hili_session_pipe_cleanup(vlib_main_t *vm, session_worker_t *wrk, session_t *hili_ss)
{
	FUNC_TRACE;
...
always_inline int
hili_session_tqs_empty(session_worker_t *wrk)
{
	if (TAILQ_EMPTY(&wrk->session_ue_queue) &&
			TAILQ_EMPTY(&wrk->session_txe_queue) &&
				TAILQ_EMPTY(&wrk->session_pipe_ue_queue) &&
					TAILQ_EMPTY(&wrk->session_pipe_de_queue) &&
						TAILQ_EMPTY(&wrk->session_hclosee_queue) &&
							TAILQ_EMPTY(&wrk->session_terme_queue) &&
									TAILQ_EMPTY(&wrk->session_ptxe_queue)) {
		return 1;
	}
...
always_inline int
session_queue_node_process_events(vlib_main_t * vm, vlib_node_runtime_t * node, session_main_t *smm, session_worker_t *wrk)
{
	int n_tx_packets = 0;

	/*Session app events*/
	while (!TAILQ_EMPTY(&wrk->session_ue_queue)) {
		session_t *hili_ss = TAILQ_FIRST(&wrk->session_ue_queue);
		TIMER_TAILQ_STOP(hili_ss, wrk->session_ue_queue, session_ue);
		if (hili_ss->rx_cb_func) {
			hili_ss->read = 1;
			hili_ss->rx_cb_func(hili_ss);
		} else {
			int len;
			vlib_buffer_t *vb;
			len = hili_session_read(vm, hili_ss, &vb);
			if (len > 0) {
				vlib_buffer_hili_free_one_buffer(vm, vb);
			}
		}
	}

	/* TCP terminate events*/
	while (!TAILQ_EMPTY(&wrk->session_terme_queue)) {
		session_t *hili_ss = TAILQ_FIRST(&wrk->session_terme_queue);
		...
		if (hili_ss->hili_flags & SS_TCP_APPTERM) {
			...
			LINE_TRACE;
			stream_session_terminate_transport(hili_ss);
			hili_session_stop_tiemr_queues(hili_ss, wrk);
	...
	while (!TAILQ_EMPTY(&wrk->session_ptxe_queue)) {
		session_t *hili_ss = TAILQ_FIRST(&wrk->session_ptxe_queue);
		TIMER_TAILQ_STOP(hili_ss, wrk->session_ptxe_queue, session_ptxe);
		hili_ss->hili_flags |= SS_TCP_APPWRITE;
		session_node_print("Come here");
		session_log_debug("Come here");
		hili_add_session_to_tcp_txq(wrk, hili_ss);
	}

	/* TCP tx events*/
	while (!TAILQ_EMPTY(&wrk->session_txe_queue)) {
		session_t *hili_ss = TAILQ_FIRST(&wrk->session_txe_queue);
		TIMER_TAILQ_STOP(hili_ss, wrk->session_txe_queue, session_txe);
		if (hili_ss->hili_flags & SS_TCP_APPWRITE) {
			LINE_TRACE;
			hili_ss->hili_flags &= ~SS_TCP_APPWRITE;
			/* Don't try to send more that one frame per dispatch cycle */
			if (n_tx_packets == VLIB_FRAME_SIZE) {
				/*The APPCLOSE flag would be know by FIFO_EVENT_APP_DIRECT_TX handler*/
				session_node_print("Come here");
				session_log_debug("Come here");
				hili_add_session_to_tcp_pending_txq(wrk, hili_ss);
				continue;
			}
			...
			* different nodes */
			rv = (smm->session_tx_fns[hili_ss->session_type]) (vm, node, wrk, NULL, &n_tx_packets);
			if (PREDICT_TRUE (rv == SESSION_TX_OK)) {
				/*walk down to check APPCLOSE flag*/
	...
	/* TCP close events*/
	while (!TAILQ_EMPTY(&wrk->session_hclosee_queue)) {
		session_t *hili_ss = TAILQ_FIRST(&wrk->session_hclosee_queue);
		TIMER_TAILQ_STOP(hili_ss, wrk->session_hclosee_queue, session_hclosee);
		if (hili_ss->hili_flags & SS_TCP_APPCLOSE) {
			LINE_TRACE;
			session_close(hili_ss);
		}
	}

	/*Pipe session app events*/
	while (!TAILQ_EMPTY(&wrk->session_pipe_ue_queue)) {
		session_t *hili_ss = TAILQ_FIRST(&wrk->session_pipe_ue_queue);
	...
	/*Process pipe del requests*/
	while (!TAILQ_EMPTY(&wrk->session_pipe_de_queue)) {
		session_t *hili_ss = TAILQ_FIRST(&wrk->session_pipe_de_queue);
		TIMER_TAILQ_STOP(hili_ss, wrk->session_pipe_de_queue, ss_pipe_de);
		hili_session_pipe_cleanup(vm, wrk, hili_ss);
		session_free(hili_ss);
	}
	return n_tx_packets;
}
		       ...
		       vlib_frame_t * frame)
{
  ...
  /*三类事件*/
  if (PREDICT_FALSE (!n_events) && hili_session_tqs_empty(wrk)) {
	  return 0;
  } else if (PREDICT_FALSE (!n_events)) {
	  return session_queue_node_process_events(vm, node, smm, wrk);
  }
  ...
  for (i = 0; i < n_events; i++)
	...
	case SESSION_CTRL_EVT_CLOSE:
	   ...
	   * dispatch cycles */
	  if (!e->postponed
	      || (e->postponed < 200 && session_bytes_in_buffer_storage (s, TX_BUFFER)))
	    {
	      e->postponed += 1;
  ...
  hili_event_tx_packets = session_queue_node_process_events(vm, node, smm, wrk);
  vlib_node_increment_counter (vm, session_queue_node.index,
			       SESSION_QUEUE_ERROR_TX, n_tx_packets);

  SESSION_EVT_DBG (SESSION_EVT_DISPATCH_END, smm, thread_index);

  return n_tx_packets + hili_event_tx_packets;
}
